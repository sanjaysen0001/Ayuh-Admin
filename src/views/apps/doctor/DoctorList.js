...
class DoctorList extends React.Component {
    ...
    columnDefs: [
...

      {
        ...
        cellRendererFramework: (params) => {
          return (
            <div>
              <img
                src={`${baseURLImage}/${params.data.image}`}
                alt=""
                width="40"
                height="40"
              />
...

      {
        ...
        cellRendererFramework: (params) => {
          return (
            <div>
              <span>{params?.data?.schedule}</span>
            </div>
          );
        },
      },
      {
        ...
        cellRendererFramework: (params) => {
          return (
            <div>
              <span>{params?.data?.consulting}</span>
            </div>
          );
        },
      },

      {
        ...
        field: "physicianVisitFees",
        filter: true,
        width: 150,
        cellRendererFramework: (params) => {
          return (
            <div>
              <span>{params?.data?.physicianVisitFees}</span>
            </div>
          );
        },
      },
              ...
              <Route
                render={({ history }) => (
                  <Eye
                    ...
                    color="green"
                    onClick={() =>
                      history.push(`/userList/viewdoctor/${params.data._id}`)
                    }
                  />
                )}
              />
              ...
              <Route
                render={({ history }) => (
                  <Edit
                    ...
                    color="blue"
                    onClick={() =>
                      history.push(`/userList/EditDoctor/${params.data._id}`)
                    }
                  />
                )}
              />
  ...
  async componentDidMount() {
    await axiosConfig.get(`/doctorPanel/viewAll`).then((response) => {
